...
const getPerformanceTrends = async (schoolId, academicYear, classId) => {
  ...
  const trends = await Promise.all(
    terms.map(async (term) => {
      ...
      return {
        term: { id: term.id, academicYear: term.academicYear, termNumber: term.termNumber },
...
// ─── NEW: Platform Analytics for Super Admin ──────────────────
const getPlatformAnalytics = async () => {
  ...
  // Compute average attendance across all schools
  let avgAttendance = 0;
  const attendanceStats = await prisma.attendance.groupBy({
    by: ["status"],
    _count: { id: true },
  });
  const totalAttendances = attendanceStats.reduce((sum, s) => sum + s._count.id, 0);
  const present = attendanceStats.find((s) => s.status === "PRESENT")?._count.id || 0;
  if (totalAttendances > 0) {
    avgAttendance = parseFloat(((present / totalAttendances) * 100).toFixed(1));
  }
