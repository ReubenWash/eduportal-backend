...
const getScoreTemplateData = async (schoolId, { classId, subjectId, termId }) => {
  const enrollments = await prisma.enrollment.findMany({
    where: { classId, termId, student: { schoolId } },
  ...
  return enrollments.map((e) => {
    const existing = scoreByStudent[e.studentId];
    return {
...
const bulkImportScoresFromExcelRows = async (schoolId, subjectId, termId, rows) => {
  ...
  for (const row of rows) {
    try {
      ...
      const student = await prisma.student.findFirst({ where: { id: studentId, schoolId } });
      if (!student) { failed.push({ row, error: "Student not found in this school." }); continue; }

      const ca1 = row.ca1 !== "" && row.ca1 != null ? Number(row.ca1) : null;
      const ca2 = row.ca2 !== "" && row.ca2 != null ? Number(row.ca2) : null;
      const ca3 = row.ca3 !== "" && row.ca3 != null ? Number(row.ca3) : null;
      const examScore = row.examScore !== "" && row.examScore != null ? Number(row.examScore) : null;

      if ([ca1, ca2, ca3, examScore].every((v) => v === null)) { skipped++; continue; }

      // Reuses the SAME computeScore() that submitScore() uses.
      const computed = computeScore({ ca1, ca2, ca3, examScore });

      await prisma.score.upsert({
        where: { studentId_subjectId_termId: { studentId, subjectId, termId } },
        create: { studentId, subjectId, termId, ca1, ca2, ca3, caTotal: computed.caTotal, examScore, total: computed.total, grade: computed.grade, remark: computed.remark },
        update: { ca1, ca2, ca3, caTotal: computed.caTotal, examScore, total: computed.total, grade: computed.grade, remark: computed.remark },
      });
...
module.exports = {
  ...
  getClassSummary,
  getSubmissionStatus,
  getScoreTemplateData,           // NEW
  bulkImportScoresFromExcelRows,  // NEW
};