...
const getReports = async (schoolId, { classId, termId, studentId, status } = {}) => {
  const where = {
    student: { schoolId, ...(classId && { enrollments: { some: { classId } } }) },
    ...(termId && { termId }),
    ...(studentId && { studentId }),
    ...(status && { status }),
  };
  ...
  return prisma.report.findMany({
    ...
    include: {
      student: {
        select: { id: true, firstName: true, lastName: true, otherNames: true, studentNumber: true, photoUrl: true },
      },
      term: { select: { id: true, academicYear: true, termNumber: true } },
    },
